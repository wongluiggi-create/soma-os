...
const Habitos = () => {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'usuarios', uid, 'habitos'), orderBy('createdAt', 'desc'));
  ...
  const toggleHabit = async (habitId, dateStr) => {
    ...
    if (habit && auth.currentUser) {
      ...
      await updateDoc(doc(db, 'usuarios', auth.currentUser.uid, 'habitos', habitId), {
  ...
  const deleteHabit = async (habitId) => {
    if (auth.currentUser && window.confirm("¿Seguro que deseas eliminar este hábito?")) {
      await deleteDoc(doc(db, 'usuarios', auth.currentUser.uid, 'habitos', habitId));
    }
  };
  ...
  const handleCreateHabit = async () => {
    ...
    await addDoc(collection(db, 'usuarios', auth.currentUser.uid, 'habitos'), nuevoHabito);
              ...
              <tr key={habit.id}>
                <td className="habit-info-cell">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button className="btn-icon-action delete-icon" onClick={() => deleteHabit(habit.id)} title="Eliminar Hábito">
